        ...
        <>
          ...
          >
            <div className="p-6 border-b border-white/10 flex items-center justify-between">
              <div>
                <h2 className="text-xl font-retro text-white flex items-center gap-3 uppercase italic tracking-tighter">
                  <Save className="w-5 h-5 text-cyan-electric" />
                  ESTADOS DE GUARDADO
                </h2>
                <p className="text-[10px] font-retro text-zinc-500 mt-1 uppercase tracking-widest">Gestiona tu progreso táctico</p>
              </div>
              <button 
                onClick={onClose}
            ...
            <div className="p-6 border-b border-white/10 space-y-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] font-retro text-zinc-500 uppercase tracking-widest">Slots de Memoria</span>
                <span className={`text-[10px] font-retro uppercase tracking-widest ${states.length >= maxSlots ? 'text-rose-500' : 'text-emerald-500'}`}>
              ...
              <button
                onClick={handleSave}
                disabled={isSaving}
                className={`w-full py-4 rounded-xl font-retro text-xs uppercase tracking-widest flex items-center justify-center gap-3 transition-all ${
                  isSaving 
              ...
              <div className="relative group">
                {!hasCloudSync && (
                  <div className="absolute inset-0 bg-black/40 backdrop-blur-[1px] rounded-xl z-10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <Link 
                      to="/marketplace" 
                      className="px-4 py-2 bg-cyan-electric text-black text-[10px] font-retro uppercase tracking-widest rounded-lg flex items-center gap-2"
                    >
                ...
                <button
                  ...
                  className={`w-full py-3 rounded-xl font-retro text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 transition-all border ${
                    isSyncing 
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 hide-scrollbar">
              {states.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-zinc-700">
                  ...
                  <p className="text-[10px] font-retro uppercase tracking-widest">No hay registros de guardado</p>
                </div>
              ) : (
                states.map((state) => (
                  <div 
                  ...
                  >
                    <div className="aspect-video bg-black relative">
                      {state.screenshot ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-zinc-800 font-retro text-[10px] uppercase tracking-widest">SIN PREVISUALIZACIÓN</div>
                      )}
                      <div className="absolute top-3 left-3 bg-black/80 backdrop-blur-md px-2 py-1 rounded-lg text-[8px] font-retro text-cyan-electric border border-white/10 uppercase tracking-widest">
                        {state.type}
                      </div>
                    </div>
                    <div className="p-4 flex items-center justify-between bg-zinc-900/80">
                      <div className="flex items-center gap-2 text-[10px] font-retro text-zinc-400 uppercase tracking-widest">
