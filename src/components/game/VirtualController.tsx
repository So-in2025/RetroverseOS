...
export default function VirtualController({ isVisible }: VirtualControllerProps) {
  ...
  const dpadBase = "bg-white/20 active:bg-white/40 backdrop-blur-md border border-white/10";
  ...
  return (
    <div className="absolute inset-x-0 bottom-0 h-40 md:h-56 pointer-events-none z-50 flex justify-between items-end px-2 pb-4 md:pb-6 select-none touch-none">
      ...
      <div className="flex gap-2 md:gap-4 mb-2 md:mb-8 pointer-events-auto z-50">
        <button 
          className="px-3 py-1 md:px-5 md:py-2 bg-black/60 active:bg-white/20 backdrop-blur-md rounded-full border border-white/10 text-[8px] md:text-[10px] font-black text-zinc-400 uppercase tracking-widest transition-colors"
      ...
      <div className="relative w-32 h-32 md:w-48 md:h-48 pointer-events-auto">
        ...
        {/* X (Top) */}
        <button 
          className={`${btnBase} ${actionBtnBase} top-0 md:top-2 left-1/2 -translate-x-1/2 w-12 h-12 md:w-16 md:h-16 bg-blue-500/20 border-blue-500/30 text-blue-400 font-black text-base md:text-xl`}
        ...
        {/* B (Bottom) */}
        <button 
          className={`${btnBase} ${actionBtnBase} bottom-0 md:bottom-2 left-1/2 -translate-x-1/2 w-12 h-12 md:w-16 md:h-16 bg-yellow-500/20 border-yellow-500/30 text-yellow-400 font-black text-base md:text-xl`}
        ...
        {/* Y (Left) */}
        <button 
          className={`${btnBase} ${actionBtnBase} left-0 md:left-2 top-1/2 -translate-y-1/2 w-12 h-12 md:w-16 md:h-16 bg-emerald-500/20 border-emerald-500/30 text-emerald-400 font-black text-base md:text-xl`}
        ...
        {/* A (Right) */}
        <button 
          className={`${btnBase} ${actionBtnBase} right-0 md:right-2 top-1/2 -translate-y-1/2 w-12 h-12 md:w-16 md:h-16 bg-rose-500/20 border-rose-500/30 text-rose-400 font-black text-base md:text-xl`}
